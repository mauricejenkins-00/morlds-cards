...
// All visuals are kept in a minimal black & white palette.
const defaultMorlds = [
  {
...
// Simple client-side "database" backed by localStorage
...
// Earth-like globe appearance for final formed state
...
// State for zoom + rotation
let zoomLevel = 1; // 1 = base, 0.7–1.6 range
let rotationDeg = 0;
...
function playFormationSequence() {
  ...
  formationTimeouts.push(
    setTimeout(() => {
      // Atmosphere finishes – near-Earth
      worldCore.style.backgroundImage =
  ...
  formationTimeouts.push(
    setTimeout(() => {
      // Return to the morld’s own style – "remaining needed to create Earth" completed
...
function animationLoop(timestamp) {
  ...
  if (autoRotate && !isDragging) {
    // Gentle, constant spin in the void
...
function renderTagPills(tags) {
  ...
  return tags
    .map(
      (t) =>
        `<span class="tag-pill"><span>${t}</span></span>`
    )
    .join("");
}
...
function setFullscreen(on) {
  ...
  if (on) {
    // Reset zoom a bit for a cinematic view
    setZoom(1.1);
    playFormationSequence();
  } else {
    clearFormationSequence();
    // If the sequence never finished, fall back to morld visuals.
...
function applyMorld(morld) {
  ...
  worldOrbit.style.boxShadow = `
        0 0 60px ${morld.style.orbitGlow},
        0 0 160px ${morld.style.orbitGlow} inset
      `;
  ...
  const planetSummary = morld.planet
    ? `${morld.planet.name || "—"} · ${morld.planet.radius || "—"} · ${
        morld.planet.atmosphere || "—"
      }`
    : "—";
  const backgroundSummary = morld.background
    ? `${morld.background.style || "—"} – ${
        morld.background.description || ""
      }`.trim()
    : "—";
...
// Capture card to canvas; optionally skip images to avoid tainted canvas (e.g. file:// or CORS)
...
// NFT transform toggle + GIF + mint link
...
// Zoom via wheel (desktop) and pinch (mobile browser will natively zoom page, which is fine)
// Prevent scroll from moving page while over world
...
function onPointerMove(e) {
  ...
  lastX = e.clientX;
  rotationDeg += dx * 0.35; // sensitivity
  applyOrbitTransform();
}
...
// Fullscreen toggle on planet/orbit click

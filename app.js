...
// Base URL of the app (where index.html and app.js live). Derived from this script's URL
// so it works on GitHub Pages (e.g. https://user.github.io/repo-name/) and locally.
...
const catalogUrl = APP_BASE + 'morlds-catalog.json';

function resolveMorldUrl(path) {
...
function openDetails(morld) {
  ...
  const morldUrl = resolveMorldUrl(path);
  content.innerHTML = `
    <div class="details-preview-wrap">
      <iframe src="${escapeHtml(morldUrl)}" title="${escapeHtml(morld.name)}"></iframe>
    </div>
    ...
    <a href="${escapeHtml(morldUrl)}" class="details-enter-btn" target="_blank" rel="noopener">Enter Morld →</a>
  `;
